...
export default function SchedulePage() {
  ...
  const [swapModalItem, setSwapModalItem] = useState<ScheduleItemRead | null>(null);
  ...
  const handleSelectAlternative = useCallback(
    (mealId: number) => {
      ...
      swapMutation.mutate(
        { itemId: swapModalItem.id, mealId, weekStartDate: weekStartStr },
        { onError: () => Alert.alert('Error', 'Failed to swap meal.') }
      );
      setSwapModalItem(null);
    },
    [swapModalItem, swapMutation, weekStartStr]
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <AlternativesModal
        visible={swapModalItem !== null}
        onClose={() => setSwapModalItem(null)}
        currentMealTitle={swapModalItem?.meal?.title ?? null}
        alternatives={(swapModalItem?.alternatives ?? []) as MealRead[]}
